...
;; set up from JAVA API to initialise mag
to setupAPI [initial-magnetization]
  clear-all
  set sum-of-spins initial-magnetization 
  reset-ticks
  ;; set up pathces to reflect this average
  ask patches [
     let randomNumber random-float 1 
     ifelse randomNumber < initial-magnetization
        [ set spin 1 ]  
        [ set spin 0 ]
     recolor
  ]
  ;;show initial-magnetization
  ;;set sum-of-spins sum [spin] of patches
 ;; show magnetization
end
